    ...
    describe OptionCheck do
    	...
    	describe "#validate" do
  			...
  			it "raise error when target_variable is not set" do
  				expect { option_check.validate }.to raise_error OptionCheckError, 'foo must be set'
  			end

  			it "raise error when target_variable is not set" do
  				option_check.targets = []
  				option_check.target_container.should_receive(:foo).and_return('foo')
  				expect { option_check.validate }.to raise_error OptionCheckError, 'An array of required/excluded options must be specified'
  			end
